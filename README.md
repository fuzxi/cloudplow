...
To use the post_script feature, add the key entry 'post_script' under the 'uploader' heading and set its value to the location of the script to be run after the upload is completed, e.g. '/opt/upload-notify.sh'. Note that this will run the script every time the upload task completes, even if there are no files to be uploaded.
